...
| `tag_search_delay_seconds` | `10` | 切换到下一个岗位标签前的基础等待秒数；脚本会自动增加少量随机抖动 |
...
切换标签时默认等待 `10` 秒，并带少量随机抖动，避免连续快速提交搜索。每个标签搜索后会按 `search_result_scroll_rounds` 做低频滚动扩展读取，默认 `1` 次，最多建议 `2` 次。这个设计是为了减少平台风控风险，宁可少刷新，也不持续轮询。
