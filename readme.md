...
3. 每个自定义推荐 Tab 默认低频滚动读取 10 次；滚动过程中发现多少新岗位就处理多少。
4. 推荐 Tab 全部处理后，再进入关键词标签搜索。
5. 关键词搜索每轮每个标签只搜索一次；所有标签无新岗位后进入冷却，默认 60 分钟。
6. 每个推荐源或关键词会低频滚动左侧岗位列表，默认最多 10 次，不做高频刷新。
...
每个自定义推荐 Tab 默认不按岗位数量截断，而是默认低频滚动读取 10 次；滚动过程中发现多少新岗位就处理多少：
...
| `search_result_scroll_rounds` | `10` | 每个推荐源或关键词最多低频滚动读取次数 |
