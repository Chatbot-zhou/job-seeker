...
脚本会优先滚动 BOSS 左侧岗位列表，不滚动整页。如果 `doctor` 提示最近有“滚动定位失败”，先更新油猴脚本并刷新 BOSS 页面。新版脚本会尝试岗位链接祖先容器、左侧可滚动列表和 wheel 事件兜底；仍失败时，通常是 BOSS 页面结构变更或页面上有筛选弹层遮挡，需要贴出 `doctor` 和脚本日志继续适配。
