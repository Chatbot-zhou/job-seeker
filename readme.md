...
| `search_round_cooldown_minutes` | `30` | 所有岗位标签跑完且没有新岗位后，等待多少分钟再开始下一轮搜索 |
| `tag_search_delay_seconds` | `10` | 切换到下一个岗位标签前的最小等待秒数 |
| `tag_search_delay_max_seconds` | `30` | 切换到下一个岗位标签前的最大等待秒数；每次会在最小值和最大值之间独立随机等待 |
| `search_result_scroll_rounds` | `1` | 每个标签搜索后最多低频滚动读取次数；建议 `0-2`，不要做高频滚动 |
...
当前版本采用保守账号安全策略：每轮每个岗位标签最多搜索一次。如果所有标签都没有新岗位，脚本不会继续每隔几秒刷新，而是进入搜索冷却，默认等待 `30` 分钟后再从第一个标签开始下一轮。

切换标签时默认在 `10-30` 秒之间随机等待，避免连续快速提交搜索。这个范围由 `tag_search_delay_seconds` 和 `tag_search_delay_max_seconds` 控制，人工模式的 `session` 命令可以调整。

每个标签搜索后会按 `search_result_scroll_rounds` 做低频滚动扩展读取，默认 `1` 次，最多建议 `2` 次。BOSS 搜索结果真正加载更多岗位的位置通常是左侧岗位列表，不是整个浏览器页面；当前油猴脚本会优先探测并滚动左侧岗位列。这个设计是为了减少平台风控风险，宁可少刷新，也不持续轮询。

冷却期间脚本仍然在线，只是不提交搜索、不切换标签、不刷新页面。状态面板和 `doctor` 会显示冷却结束时间和剩余时间；如需人工强制开启新一轮，可以先 `stop`，再重新 `start`。

BOSS 手机 APP 的推荐流和 Web 关键词搜索不是同一个入口。APP 里看到更多岗位，不一定代表脚本漏投；它可能来自推荐算法、地域扩展、历史行为或不同筛选条件。脚本当前只做 Web 关键词搜索，不模拟 APP 推荐流。
...
| 搜索框、搜索按钮、岗位列表元素 | 页面元素缺失最多恢复 3 次 | 仍失败则暂停，避免反复刷新 |
| 关键词搜索提交 | 失败可随页面恢复重试，但保持标签随机间隔 | 遇到平台限制立即停止 |
| 搜索结果滚动读取 | 每个标签默认 1 次，建议不超过 2 次 | 仍无新岗位则切换标签或进入冷却 |
| 职位详情页回传 | 后台详情页超时后尝试 fetch 兜底；整体最多重试 1 次 | 跳过当前岗位 |
| 岗位评分模型调用 | 3 次策略：当前配置、关闭思考、关闭思考并调整温度 | 评分失败则跳过当前岗位 |
| 打招呼聊天页打开和发送 | 首次 + 2 次重试，总计最多 3 次 | 最终失败则记录并继续或暂停 |
| 消息发送结果确认 | 等待发送结果，不盲目重复点击发送 | 结果不明确时记录失败，避免重复打招呼 |
...
这是账号安全策略。旧策略会在几秒后重新搜索所有标签，长时间运行容易形成高频提交和刷新。当前版本会进入搜索冷却，默认 `30` 分钟，只保持心跳和状态展示，不重新提交搜索。可以在人工模式输入 `session` 调整冷却分钟数、标签间隔和滚动扩展次数。

### 手机 APP 有推荐岗位，但脚本搜索不到

这是入口差异。脚本使用 Web 端关键词搜索，APP 使用推荐流，两者结果不同很正常。建议把 APP 中常见的岗位关键词和城市组合整理到 `data/cache/tags.txt`，例如 `AI开发工程师北京`、`RAG工程师西安`。不要通过缩短冷却时间来硬刷，频繁刷新更容易触发风控。
