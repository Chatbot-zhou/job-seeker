...
3. 每个自定义推荐 Tab 默认最多处理 10 个新岗位。
4. 推荐 Tab 全部处理后，再进入关键词标签搜索。
5. 关键词搜索每轮每个标签只搜索一次；所有标签无新岗位后进入冷却，默认 60 分钟。
6. 每个推荐源或关键词会低频滚动左侧岗位列表，默认最多 5 次，不做高频刷新。
7. 模型评分达到阈值、历史未重复、公司未超限后，进入聊天页发送已确认话术。
8. 聊天页发送会在同一个页面内最多尝试 3 次；仍失败则关闭当前临时页，记录失败，继续下一个岗位。
9. 如果点击发送后结果无法确认，记录 `greet_delivery_unknown`，关闭当前临时页，跳过该岗位，继续运行。
10. 登录过期、验证码、安全验证、平台额度提醒、访问过频等风险状态不会绕过，会暂停或停止并提示人工处理。
...
- 用户画像保存在 `data/cache/user_detail.txt`。
...
CLI 会确认本轮岗位标签和本轮打招呼上限，然后允许脚本运行。
...
每个自定义推荐 Tab 默认最多处理 10 个新岗位：

```json
"preferred_feed_max_jobs_per_tab": 10
```

设置为 `0` 表示不限制单个 Tab 的处理数量，但仍受本轮上限、每日安全上限、历史去重和平台风控限制。
...
| 配置 | 默认值 | 说明 |
| --- | --- | --- |
| `search_round_cooldown_minutes` | `60` | 一轮关键词全部无新岗位后的冷却分钟数 |
...
| `search_result_scroll_rounds` | `5` | 每个推荐源或关键词最多低频滚动读取次数 |

冷却期间脚本仍在线，只是不提交搜索、不切换标签、不刷新页面。`status` 和 `doctor` 会显示冷却状态。
...
BOSS 每日沟通额度约 150 次，但约 120 次后可能出现“剩余次数”提醒。该提醒会改变页面 DOM，继续自动查找发送按钮容易造成连续错误。
...
脚本当天自动打招呼达到 120 次会主动停止。如果当天用户手动沟通过，平台提醒可能提前出现；脚本检测到“今天剩余”“剩余次数”“今日还可”等额度提醒后，也会立即停止自动化，不会刷新页面反复重试。
...
| `data/cache/user_detail.txt` | 用户画像 |
...
当前版本会在同一个聊天页内尝试 3 次发送。仍失败时会关闭当前临时页，记录失败或 `greet_delivery_unknown`，然后继续下一个岗位，不会无限开新聊天页。
...
这是账号安全策略。旧策略会每隔几秒循环搜索，容易触发风控。当前版本默认冷却 60 分钟，只保持心跳和状态，不重新提交搜索。
