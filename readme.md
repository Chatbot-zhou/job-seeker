...
本项目基于 MIT 协议进行修改和再发布，保留原作者版权声明，并由 Chatbot-Zhou 维护当前版本。
...
1. 配置模型来源、Ollama 或 OpenAI-compatible API、模型名、端口、阈值、每日上限、黑名单和自动化模式。
2. 输入 PDF 简历路径。
3. 系统提取 PDF 文本并生成 `data/resume/resume.md`。
4. 系统打开编辑器，用户确认并保存简历内容。
5. 系统分别生成职位标签和用户详情。
6. 系统打开编辑器，用户确认用户详情内容。
...
2. 打开或刷新 BOSS 搜索页，等待 CLI 显示脚本心跳。
3. 在 CLI 输入 `start` 开始自动搜索、分析职位和打招呼。
4. 使用 `pause` 暂停下一步动作，使用 `stop` 停止自动化。

脚本检测到安全验证、访问异常、登录过期或明确的滑动/图形验证码页面时，会暂停并提示人工处理；不会尝试绕过平台验证。

常见排查：

- 如果 CLI 没有出现 `脚本就绪: 2026.06-cli-autogreet.15`，说明油猴脚本没有更新或没有连接到本地 API；优先运行 `script` 并通过本地安装地址更新。
- 如果出现 `未匹配页面路径: /xian/` 这类城市首页日志，请更新脚本到 `2026.06-cli-autogreet.15`；新版会自动进入标准职位搜索页。
- 如果出现 `接收超时: get-job-info`，请更新脚本到 `2026.06-cli-autogreet.15`；新版把详情页来源有效期放宽到 120 秒，并且脚本打开的详情窗口会按窗口名回传职位信息。
- 如果 CLI 报 `验证码` 但页面肉眼没有验证码，请更新到 `2026.06-cli-autogreet.15`；新版不再因为普通“验证码”文案误判，只在明确的验证/风控页面暂停。
- 油猴窗口的 `[进度]` 只在职位处理完成后更新；平均耗时按已完成岗位计算，累计耗时从本轮真正启动开始计算。
- 岗位分析当前使用单次三项评分，输入为岗位详情和已确认用户画像，模型必须严格返回“学历专业/技术栈/项目经验”三行分数，最终匹配度由系统加权计算。
- Ollama 岗位评分会使用 `think: false` 直接获取三行分数，避免小型思考模型长时间展开推理；画像和话术生成仍按配置决定是否展示思考过程。
...
- 如果 CLI 多次显示 `发现职位链接 N 个，新职位 0 个`，说明当前搜索结果已经没有未处理的新职位，脚本会结束本轮浏览。
