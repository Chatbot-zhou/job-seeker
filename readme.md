...
4. 两个平台、启动预热和配置生成共用同一个模型执行门，默认一次只运行一个模型任务，避免本地设备同时加载多个请求。
5. BOSS 在列表不足时使用自适应滚动；智联处理完当前页后定位并点击“下一页”，通过页码、网址和岗位集合验证翻页结果。智联页面刷新后以当前真实页为准，旧翻页次数会在页码或岗位集合不一致时归零，不会自动追到刷新前的页码。
...
启动时的模型预热也会经过同一个执行门，不会再与 BOSS 或智联的岗位评分同时抢占模型。若 Ollama 拒绝连接，系统会立即暂停全局任务，避免两个平台继续消耗岗位并重复失败。
...
页面面板的“结束”只停止当前平台；“开始”会检查模型和必要配置后创建新运行。全局模型故障仍会同时暂停两个平台，模型恢复后可以从任一平台面板重新开始。
...
- BOSS 与智联现在是两个独立用户脚本；如果浏览器中仍有旧版同时匹配两个网站的 `Job Seeker`，请更新旧脚本并分别安装两个新入口，避免重复运行。
...
开启后会提示重启浏览器或重新加载扩展，按提示完成后刷新 BOSS / 智联页面，再执行 `status` 确认已连接。若面板仍未出现，回到 Tampermonkey 管理页确认脚本已启用，并按需更新脚本后再次刷新页面。
...
先执行 `status`。BOSS 重点查看滚动模式、目标、轮次和岗位数量变化；智联重点查看当前真实页码、成功翻页次数、下一页结果和翻页前后的岗位数量。刷新智联页面后看到翻页次数归零属于保守恢复，不是丢失进度；近期岗位历史会阻止同一批岗位被重复评分。如果智联页面能够人工点击下一页但系统没有翻页，优先更新用户脚本并刷新页面；如果已经显示下一页禁用、列表耗尽或进入冷却，则属于正常状态。
